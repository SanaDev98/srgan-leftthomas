...
if __name__ == '__main__':
    ...
    TRAIN_SAMPLE_COUNT = opt.train_sample_count
    
    train_set = TrainDatasetFromFolder(os.path.join(base_path, "data/train"), crop_size=CROP_SIZE, upscale_factor=UPSCALE_FACTOR)
    
    if TRAIN_SAMPLE_COUNT and len(train_set) > TRAIN_SAMPLE_COUNT:
        indices = random.sample(range(len(train_set)), TRAIN_SAMPLE_COUNT)
        train_set = Subset(train_set, indices)
    
    val_set = ValDatasetFromFolder(os.path.join(base_path, "data/val"), upscale_factor=UPSCALE_FACTOR)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        torch.save(netG.state_dict(), os.path.join(save_dir, 'netG_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch)))
        torch.save(netD.state_dict(), os.path.join(save_dir, 'netD_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch)))
        ...
        perf_df.to_csv(os.path.join(stats_dir, 'performance_metrics.csv'), index=False)
        
        # Save batch metrics
        with open(os.path.join(stats_dir, f'epoch_{epoch}_batch_metrics.json'), 'w') as f:
        ...
        if epoch % 10 == 0 and epoch != 0:
            ...
            out_path = os.path.join(stats_dir, 'srf_' + str(UPSCALE_FACTOR) + '_train_results.csv')
            data_frame.to_csv(out_path, index_label='Epoch')
